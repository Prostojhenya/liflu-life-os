...
export const CreateHub: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col px-5 pt-4 pb-2 overflow-hidden">
      {/* AI Icon */}
      <div className="flex justify-center mb-3 shrink-0">
        <div className="w-14 h-14 rounded-full bg-accent-purple/20 border-2 border-accent-purple flex items-center justify-center">
      ...
      <div className="mb-3 relative shrink-0">
        <textarea
          ...
          className="w-full bg-[#150a24] border border-white/10 rounded-2xl p-3 pr-10 focus:outline-none focus:border-accent-purple transition-all text-sm text-white placeholder:text-[#8b7ca8]/50 font-display resize-none"
          rows={2}
        />
      ...
      <div className="mb-3 shrink-0">
        <p className="text-[10px] text-[#8b7ca8] font-black uppercase tracking-wider mb-1.5 font-display">
          Примеры:
        </p>
        <div className="space-y-1">
          {selectedType === 'task' && [
            ...
            <button
              ...
              className="block w-full text-left px-3 py-1.5 text-xs text-accent-purple font-display hover:bg-accent-purple/10 rounded-xl transition-colors"
            >
              {example}
            </button>
          ))}
          ...
          {!selectedType && [
            ...
            <div
              key={i}
              className="block w-full text-left px-3 py-1.5 text-xs text-[#8b7ca8] font-display rounded-xl"
            >
              {example}
            </div>
          ))}
        ...
        {TASK_TYPES.map((type) => {
          ...
          return (
            <motion.button
              ...
              className={`py-3 px-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center ${
                isSelected
      ...
      <button
        ...
        className="w-full shrink-0 bg-accent-purple text-white py-3.5 rounded-2xl font-black uppercase tracking-wider font-display flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(139,92,246,0.4)] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm"
      >
        {isCreating ? (
